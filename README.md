...
The current prototype data contract is documented in [docs/data-contract.md](docs/data-contract.md), defined in [src/data/finance.ts](src/data/finance.ts), and accessed through the session-only repository in [src/data/financeRepository.ts](src/data/financeRepository.ts). It is not durable storage or cross-device synchronization.
