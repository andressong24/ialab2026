...
- The scaffold has no authentication, device persistence, bank connections, receipt scanning, AI, or export services. The canonical prototype finance fixture is `src/data/finance.ts`, documented in `docs/data-contract.md`; session-only edits belong in `src/state/FinanceStore.tsx` and reset on reload or app restart. Do not present a placeholder operation as successful processing or durable storage.
