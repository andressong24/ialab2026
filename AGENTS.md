...
- The scaffold has no authentication, durable persistence, bank connections, receipt scanning, AI, or export services. The canonical prototype finance contract is `src/data/finance.ts`, and its session-only repository is `src/data/financeRepository.ts`; both are documented in `docs/data-contract.md`. Use `financeRepository` for finance reads and writes instead of mutating the fixture directly. Onboarding session edits belong in `src/state/FinanceStore.tsx`; both reset on reload or app restart. Do not present session-only writes as durable storage.
